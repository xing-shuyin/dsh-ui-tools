...
- **文件提及**:点「＋」把文件/文件夹/选中行加入输入框上方的提及条(与输入框自动对齐),发送问题时**每条引用作为一条独立消息、只带路径**,和问题一起发送给 Agent —— 无需在输入框里 @ 文件,Agent 按需用 read 工具读取内容(不附带文件内容,像其他 Agent 的引用方式)。
...
- **引用只发路径**:点「＋」/「🔗 引用」/选中行后,下一条提问会携带**每条文件引用作为一条独立消息、只含路径**(如 `[文件] \`src/a.ts\`` / `[引用] \`docs\``),**不会附带文件内容** —— Agent 需要内容时自己用 read 工具按需读取,和 Claude Code / Cursor 等 Agent 的引用方式一致,省 token 且不会被无关内容干扰。
- **更新后需重启**:host 端改动(安装/更新插件后)需要**重启 `dsh web`** 才生效;仅刷新页面只对 client 端改动有效。
- **引用消息不在会话气泡中显示**:引用消息注入在模型输入层,对话记录里仍然只显示你的提问文本(与旧版本行为一致)。
